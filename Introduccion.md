...
El programa necesita implementar una clase con los siguientes atributos.  
  1. peso: Arreglo con el modelo de pesos.  
  2. bias: El bias del termino.

El programa también necesita unos métodos especificos.  
  1. fit(): Entrena el modelo con la información de entrada.  
  2. predict(): Predice la salida.  
  3. score(): Computa el error de la predicción  
  4. data_scaling(): Escala los valores del set de data

El documento especifica que el pseudocódigo para probar los modelo de regresión linear multiple se debería ver así:  
...
## Uso del programa con bases de datos entregadas
...
La pendiente de la recta se calcula agarrando el valor (***p***)

Σ(x<sub>i</sub> - promedio de los valores de x) * (y<sub>i</sub> - el promedio de los valores de y)  
dividido lo por  
Σ(x<sub>i</sub> menos el promedio de los valores x)<sup>2</sup>  

El intecepto se calcula

***p*** * el promedio de x - el promedio de y  

Para calcular el error de la predicción solo necesitas  
Primero:  
Σ( y - y<sub>i</sub> ) a la dos
Dos:  
